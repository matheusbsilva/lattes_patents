...
ReadCultivarProtegida <- function(path){
  setwd(path)
  all.xmls <- list.files(".")
  df_cultivar_proteg <- data.frame()
  ...
  for (path.xml in all.xmls) {
    ...
    cultivares <- root[["PRODUCAO-TECNICA"]]["CULTIVAR-PROTEGIDA", all = TRUE]
    nome_autores <- "VAZIO"
    ...
    for (cultivar in cultivares) {
      ...
      df_cultivar_proteg <- rbind(df_cultivar_proteg, df_aux)
    }
  }
  rm(df_aux)
  
  df_cultivar_proteg$id_patente <- RemoveNonAlNum(df_cultivar_proteg$id_patente)
