...
ReadSoftware <- function(path) {
  ...
  for (path.xml in all.xmls) {
    ...
    marcas <- root[["PRODUCAO-TECNICA"]]["SOFTWARE", all = TRUE]
    nome_autores <- "VAZIO"
    ...
    for (marca in marcas) {
      ...
      df_aux <- data.frame(
        ...
        titulo_software = Validate(marca[[DADOS_BASICOS]], "TITULO-DO-SOFTWARE"),
        ano_desenvolvimento = Validate(marca[[DADOS_BASICOS]], "ANO"),
        pais = Validate(marca[[DADOS_BASICOS]], "PAIS"),
        idioma = Validate(marca[[DADOS_BASICOS]], "IDIOMA"),
        flag_relevancia = Validate(marca[[DADOS_BASICOS]], "FLAG-RELEVANCIA"),
        doi = Validate(marca[[DADOS_BASICOS]], "FLAG-RELEVANCIA"),
        meio_de_divulgacao = Validate(patente[[DADOS_BASICOS]], "MEIO-DE-DIVULGACAO"),
        titulo_ingles = Validate(marca[[DADOS_BASICOS]], "TITULO-DO-SOFTWARE-INGLES"),
        flag_potencial_inovacao = Validate(marca[[DADOS_BASICOS]], "FLAG-POTENCIAL-INOVACAO"),
        flag_divulgacao_cientifica = Validate(marca[[DADOS_BASICOS]], "FLAG-DIVULGACAO-CIENTIFICA"),
        
        # TAG: DETALHAMENTO-DO-SOFTWARE
        plataforma = Validate(marca[[DETALHAMENTO]], "PLATAFORMA"),
        disponibilidade = Validate(marca[[DETALHAMENTO]], "DISPONIBILIDADE"),
        finalidade = Validate(marca[[DETALHAMENTO]], "FINALIDADE"),
        finalidade_ingles = Validate(marca[[DETALHAMENTO]], "FINALIDADE-INGLES"),
        ambiente = Validate(marca[[DETALHAMENTO]], "AMBIENTE"),
        instituicao_financiadora = Validate(patente[[DETALHAMENTO]], "INSTITUICAO-FINANCIADORA"),
